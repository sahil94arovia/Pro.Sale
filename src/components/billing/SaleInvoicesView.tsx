...
export const SaleInvoicesView: React.FC<SaleInvoicesViewProps> = ({
  ...
  return (
    <div className="space-y-6 max-w-[1550px] mx-auto pb-16 font-sans text-xs text-slate-900">
      {/* ========================================================================= */}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-lg font-bold text-slate-900 tracking-tight">Sale Invoices Register</h1>
            <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 text-[11px] font-semibold tabular-nums">
              {metrics.count} Bills
            </span>
          </div>
        ...
        <div className="flex items-center space-x-2 flex-wrap">
          <button
            ...
            className="h-8 px-3 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-medium text-xs shadow-xs transition-colors flex items-center space-x-1.5 cursor-pointer"
            title="Download CSV"
          >
          ...
          <button
            ...
            className="h-8 px-4 rounded-lg bg-[#0071e3] hover:bg-[#0077ed] active:scale-[0.99] text-white font-semibold text-xs shadow-xs transition-all flex items-center space-x-1.5 cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-xs">
          ...
          <div className="mt-2">
            <span className="text-xl font-bold tabular-nums text-slate-900 tracking-tight">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-xs">
          ...
          <div className="mt-2">
            <span className="text-xl font-bold tabular-nums text-rose-600 tracking-tight">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-xs col-span-2 sm:col-span-1">
          ...
          <div className="mt-2">
            <span className="text-xl font-bold tabular-nums text-slate-800 tracking-tight">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-xs space-y-3.5">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 min-w-[260px]">
            ...
            <input
              ...
              className="w-full h-8 pl-8 pr-7 bg-slate-50/70 rounded-lg border border-slate-200 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-900 focus:bg-white transition-all"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <div className="flex items-center space-x-1.5 shrink-0">
            ...
            <select
              ...
              className="h-7 px-2.5 bg-slate-50 rounded-md border border-slate-200 text-xs font-medium text-slate-700 focus:outline-none focus:border-slate-900 cursor-pointer"
            >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-100">
          {/* Quick Date Presets */}
          <div className="flex items-center space-x-1 overflow-x-auto py-0.5 no-scrollbar">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mr-1 shrink-0 flex items-center space-x-1">
          ...
          <div className="flex items-center space-x-2 flex-wrap">
            ...
            {datePreset === 'CUSTOM_DAY' && (
              <div className="flex items-center space-x-1.5 animate-in fade-in duration-150">
                ...
                <input
                  ...
                  className="h-7 px-2 bg-slate-50 rounded-md border border-slate-300 text-xs text-slate-800 focus:outline-none focus:border-slate-900"
                />
              </div>
            )}
            ...
            {hasActiveFilters && (
              <button
                ...
                className="h-7 px-2 text-[11px] text-slate-500 hover:text-slate-900 font-semibold flex items-center space-x-1 transition-colors cursor-pointer"
              >
      ...
      {/* ========================================================================= */}
      {/* 4. SALE INVOICES DATA TABLE                                               */}
      {/* ========================================================================= */}
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-xs overflow-hidden flex flex-col">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50/80 text-slate-500 font-semibold border-b border-slate-200 text-[10px] tracking-wider uppercase">
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {sortedInvoices.length === 0 ? (
                <tr>
                  <td colSpan={10} className="py-14 text-center text-slate-400 font-sans">
                    <div className="max-w-sm mx-auto space-y-3">
                      ...
                      <p className="text-sm font-semibold text-slate-700">No sale bills found</p>
                      ...
                      {hasActiveFilters ? (
                        <button
                          type="button"
                          onClick={handleResetFilters}
                          className="px-3.5 py-1.5 rounded-lg border border-slate-300 bg-white text-slate-700 font-medium text-xs hover:bg-slate-50 cursor-pointer"
                        >
                          Clear Filters
                        </button>
                      ) : (
                        <button
                          type="button"
                          onClick={onOpenNewSale}
                          className="px-4 py-1.5 rounded-lg bg-[#0071e3] text-white font-medium text-xs hover:bg-[#0077ed] cursor-pointer"
                        >
                ...
                sortedInvoices.map((inv) => {
                  ...
                  const isCredit = inv.saleType === 'CREDIT' || inv.balanceAmount > 0;
                  const isOverdue =
                    !isPaid &&
                    inv.dueDate &&
                    ...
                    >
                      ...
                      <td className="py-3 px-3 whitespace-nowrap">
                        <button
                          ...
                          className="font-bold text-slate-900 hover:text-blue-600 transition-colors text-left flex items-center space-x-1 cursor-pointer"
                      ...
                      <td className="py-3 px-4">
                        ...
                        <div className="text-[11px] text-slate-500 flex items-center space-x-2 mt-0.5">
                          ...
                          {inv.customer?.gstin && (
                            <span className="px-1.5 py-0.2 rounded bg-slate-100 text-[9px] text-slate-600 font-mono">
                      ...
                      <td className="py-3 px-3 text-center whitespace-nowrap">
                        ...
                        {inv.paymentMode && inv.paymentMode !== 'CREDIT' && (
                          <span className="text-[9px] text-slate-400 block mt-0.5 uppercase">
                            {inv.paymentMode}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-3 whitespace-nowrap">
                        {inv.dueDate ? (
                          ...
                          >
                            ...
                            {isOverdue && <span className="block text-[9px] text-rose-500">Overdue</span>}
                          </span>
                        ) : (
                      ...
                      <td className="py-3 px-4 text-right whitespace-nowrap">
                        <span
                          className={`tabular-nums font-bold text-xs ${
                            inv.balanceAmount > 0 ? 'text-rose-600' : 'text-slate-400'
                          }`}
                        >
                      ...
                      <td className="py-3 px-4 text-center whitespace-nowrap no-print">
                        <div className="flex items-center justify-center space-x-1">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-md hover:bg-blue-50 text-slate-600 hover:text-blue-600 transition-colors cursor-pointer"
                            title="Edit Invoice"
                          >
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-md hover:bg-emerald-50 text-slate-600 hover:text-emerald-600 transition-colors cursor-pointer"
        ...
        {sortedInvoices.length > 0 && (
          <div className="bg-slate-50/90 border-t border-slate-200 px-5 py-3 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-600 gap-2">
            <div>
              Showing <strong className="font-semibold text-slate-900">{sortedInvoices.length}</strong> of{' '}
              <strong className="font-semibold text-slate-900">{invoices.length}</strong> total sale bills
        ...
        >
          <div className="space-y-4">
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-lg text-rose-800 text-xs leading-relaxed space-y-1">
              <div className="font-bold flex items-center space-x-1 text-rose-900">
            ...
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 space-y-1.5 text-xs text-slate-700">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-900">{deletingInvoice.invoiceNumber}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Party Name:</span>
                <span className="font-semibold text-slate-900">{deletingInvoice.customer?.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Invoice Date:</span>
                <span className="font-semibold text-slate-900">{formatDate(deletingInvoice.date)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Total Bill Amount:</span>
                <span className="font-bold text-slate-900 tabular-nums">{formatINR(deletingInvoice.grandTotal)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Items Count:</span>
                <span className="font-medium text-slate-900">{deletingInvoice.items?.length || 0} Products</span>
              </div>
            </div>
